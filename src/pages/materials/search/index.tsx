...
const App: React.FC = () => {
  ...
  const filteredData = data.filter((item:any) => {
    ...
    if (filters.keyword) {
      ...
      if (item.type === '备品备件') {
        return (
          item.details.partNumber.toLowerCase().includes(keyword) ||
          item.details.serialNumber.toLowerCase().includes(keyword) ||
          item.info.toLowerCase().includes(keyword)
        );
      } else {
        return (
          item.details.projectCode.toLowerCase().includes(keyword) ||
          item.details.projectName.toLowerCase().includes(keyword) ||
          item.info.toLowerCase().includes(keyword)
        );
      }
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render:  (_:string, record: any)  => (
        <Space size="middle">
          <Button onClick={() => openDetailModal(record)}>查看台账详情</Button>
        </Space>
      ),
    },
  ];
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Card title="物资全量查询" bordered={false}>
        {/* 筛选条件 */}
        <Card type="inner" title="筛选条件" style={{ marginBottom: '24px' }}>
          <Row gutter={[16, 16]}>
            ...
            <Col span={6}>
              <label style={{ marginRight: '8px' }}>当前状态：</label>
              <Select
                style={{ width: '150px' }}
                placeholder="请选择状态"
            ...
            <Col span={8}>
              <label style={{ marginRight: '8px' }}>入库时间：</label>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <Timeline>
                {selectedRecord.records.map((record: any, index: number) => (
                  <Timeline.Item key={index} color={record.type === '入库' ? 'green' : record.type === '出库' ? 'red' : 'blue'}>
                    <div>
