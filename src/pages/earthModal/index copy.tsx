...
const showDiv = (pos: THREE.Vector3) => {
  if (!cameraRef.current || !containerRef.current || !layerRef.current) return;
  // 将3D坐标转换为屏幕坐标
  ...
  layerRef.current.style.left = `${x - 35}px`;
  layerRef.current.style.top = `${y - 110}px`;
  
  // 添加动画效果
  layerRef.current.style.opacity = "0";
  layerRef.current.style.transform = "scale(0.5)";
  
  setTimeout(() => {
    layerRef.current!.style.transition = "all 0.3s ease-out";
    layerRef.current!.style.opacity = "1";
    layerRef.current!.style.transform = "scale(1)";
  }, 0);
};
      ...
      <div className="rightMain">
        ...
        <div className="cont">
          <ul>
            ...
            <li onClick={delMarkChange}>删除美国标记点</li>
          </ul>
        </div>
